...
def convolution_filter(img, kernels, ftype):
...
def test_convolution_filter():
    ...
    show(img)
    new_image = convolution_filter(img, blur_kernel)
    show(new_image)
    # save_as(new_image, "great_big_c_convolution_blur.jpg")

def build_kernel_table():
    kernels = {}
    kernels["blur"] = ((1, 1, 1), (1, 1, 1), (1, 1, 1))
    kernels["sharpen"] = ((0, -3, 0), (-3, 21, -3), (0, -3, 0))
    kernels["emboss"] = ((-18, -9, 0), (-9, 9, 9), (0, 9, 18))
    kernels["edge detect 1"] = ((0, 9, 0), (9, -36, 9), (0, 9, 0))
    kernels["edge detect 2"] = ((-9, -9, -9), (-9, 72, -9), (-9, -9, -9))
    return kernels
